...
declare v_day text;v_intervals jsonb;v_interval jsonb;v_start text;v_end text;v_previous_end text;
begin
  if jsonb_typeof(p_hours) is distinct from 'object'
    ...
    or jsonb_object_length(p_hours) <> 2
    or jsonb_object_length(p_hours->'days') <> 7
  ...
  foreach v_day in array array['monday','tuesday','wednesday','thursday','friday','saturday','sunday'] loop
    ...
    for v_interval in select value from jsonb_array_elements(v_intervals) loop
      ...
      if jsonb_typeof(v_interval) is distinct from 'object'
        or jsonb_object_length(v_interval) <> 2
        or v_start is null
        or v_end is null
...
create or replace function public.enforce_weekly_operating_hours() returns trigger language plpgsql as $$ begin if not public.valid_weekly_operating_hours(new.operating_hours) then raise exception 'operating hours must use valid non-overlapping same-day Pacific time intervals';end if;return new;end $$;
create trigger validate_provider_operating_hours before insert or update of operating_hours on public.provider_companies for each row execute function public.enforce_weekly_operating_hours();
